...
public class JoinsTest1 {
	public static void main(String[] args) {
		Session session = SessionUtil.getSession();
		Query query = session.createQuery("from Employee as emp LEFT JOIN emp.department as dep");
		...
		Query query1 = session.createQuery("from Department as dep RIGHT JOIN dep.employee as emp");
		List<?> list1 = query1.list();
		for (int i = 0; i < list1.size(); i++) {
			Object[] row1 = (Object[]) list1.get(i);
			Department department1 = (Department) row1[0];
			Employee empolyee1 = (Employee) row1[1];
